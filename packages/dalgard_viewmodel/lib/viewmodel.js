...
ViewModel = class ViewModel extends Base {
  constructor(view, name = view.name, id = uniqueId(), definition, options) {
    ...
    let hash_id = this.hashId(true),
        is_hcp_restore = ViewModel.restoreAfterHCP && isHCP;
...
// Export utility methods with class
defineProperties(ViewModel, {
  // Get next unique id
  uniqueId: { value: uniqueId },

  // Add a binding to the global list
  addBinding: { value: Binding.add },

  // The {{bind}} Blaze helper
  bindHelper: { value: bindHelper },

  // Register the bind helper globally and make __helpers reactive
  registerHelper: { value: registerHelper },

  // Viewmodel declaration hook
  viewmodelHook: { value: viewmodelHook }
});
